...
export class AnalyzerAgent {
    ...
    async deepAnalyze(filePath, content, quickFindings) {
...
If no issues found, respond with "NO_ISSUES_FOUND"`;
    ...
    parseFindings(filePath, response, quickFindings) {
        ...
        for (const block of blocks.slice(1)) {
            const finding = { file: filePath };
            for (const line of block.split('\n')) {
                const trimmed = line.trim();
                if (trimmed.startsWith('- Severity:')) {
                    finding.severity = trimmed.split(':')[1].trim().toLowerCase();
                }
