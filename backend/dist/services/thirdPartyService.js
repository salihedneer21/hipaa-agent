...
function stableProviderId(name, domain) {
...
export async function detectThirdPartyServices(repoPath, fileTree) {
    ...
    for (const rel of pkgFiles) {
        ...
        try {
            ...
            for (const depName of deps) {
                if (!depName)
                    continue;
                const set = depToFiles.get(depName) || new Set();
                set.add(rel);
    ...
    for (const rel of candidateFiles) {
        ...
        try {
            ...
            for (const domain of extractDomainsFromText(raw)) {
                const set = domainToFiles.get(domain) || new Set();
                set.add(rel);
...
- Keep the list focused and deduplicated (max 30 providers).`;
    ...
    try {
        ...
        for (const svc of Array.isArray(parsed.services) ? parsed.services : []) {
            ...
            if (!name)
                continue;
            const domain = normalizeDomain(svc.domain);
            const id = stableProviderId(name, domain);
            if (seen.has(id))
                continue;
            seen.add(id);
            const evidence = Array.isArray(svc.evidence)
                ? svc.evidence
                    .map((ev) => ({
                    kind: (ev?.kind || 'other'),
                    ...
                    .filter((ev) => Boolean(ev.value && ev.file))
                    .slice(0, 12)
                : [];
            detected.push({
                id,
                name,
                domain,
                category: typeof svc.category === 'string' && svc.category.trim() ? svc.category.trim() : undefined,
                evidence,
            });
        }
...
export function enrichWithLogo(service) {
    return {
        ...service,
