 ...
 */
export async function resolveInterruptedTurn(agent, originalInput, originalPreStepItems, newResponse, processedResponse, runner, state, toolErrorFormatter) {
    ...
    const functionResults = await executeFunctionToolCalls(agent, functionToolRuns, runner, state, toolErrorFormatter);
    ...
    const shellResults = shellRuns.length > 0
        ? await executeShellActions(agent, shellRuns, runner, state._context, undefined, toolErrorFormatter)
        : [];
    const applyPatchResults = applyPatchRuns.length > 0
        ? await executeApplyPatchOperations(agent, applyPatchRuns, runner, state._context, undefined, toolErrorFormatter)
        : [];
    const newItems = [];
    ...
    const additionalInterruptions = collectInterruptions([], [...computerResults, ...shellResults, ...applyPatchResults]);
 ...
 */
export async function resolveTurnAfterModelResponse(agent, originalInput, originalPreStepItems, newResponse, processedResponse, runner, state, toolErrorFormatter) {
    ...
    const [functionResults, computerResults, shellResults, applyPatchResults] = await Promise.all([
        executeFunctionToolCalls(agent, processedResponse.functions, runner, state, toolErrorFormatter),
        executeComputerActions(agent, processedResponse.computerActions, runner, state._context, undefined, toolErrorFormatter),
        executeShellActions(agent, processedResponse.shellActions, runner, state._context, undefined, toolErrorFormatter),
        executeApplyPatchOperations(agent, processedResponse.applyPatchActions, runner, state._context, undefined, toolErrorFormatter),
    ]);
    for (const result of functionResults) {
        if (result.type === 'function_output' &&
            Array.isArray(result.interruptions) &&
