...
import type { ThirdPartyBaaAvailability, ThirdPartyBaaResearch } from '../services/thirdPartyService.js';
...
const cache = new Map<string, ThirdPartyBaaResearch>();
...
function asAvailability(value: unknown): ThirdPartyBaaAvailability {
  if (value === 'yes' || value === 'no' || value === 'partial' || value === 'unknown') return value;
  return 'unknown';
}
...
export async function researchBaaForProvider(input: ResearchInput): Promise<ThirdPartyBaaResearch> {
  ...
  if (!process.env.OPENAI_API_KEY) {
    const fallback: ThirdPartyBaaResearch = {
...
- pricing: only if clearly published; otherwise null`;
  ...
  try {
    ...
    for (const item of response.output || []) {
      if (item && (item as any).type === 'web_search_call') {
        const src = (item as any).action?.sources;
        if (Array.isArray(src)) {
          for (const s of src) {
            if (s && typeof s.url === 'string' && s.url.trim()) sources.push(s.url.trim());
          }
        }
      }
    }

    const result: ThirdPartyBaaResearch = {
