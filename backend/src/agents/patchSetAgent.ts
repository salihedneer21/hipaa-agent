...
Output STRICT JSON only, matching the required schema in system instructions.`;
  ...
  private parsePatchSet(response: string): PatchSet | null {
    ...
    const operations: PatchOperation[] = operationsRaw
      .map((op: any): PatchOperation | null => {
        ...
        if ((action !== 'modify' && action !== 'add') || !filePath || !content) return null;
        return { action: action as PatchOperation['action'], path: filePath, content };
      })
      .filter((x: PatchOperation | null): x is PatchOperation => x !== null);

    const changes = Array.isArray(patchSet?.changes)
      ? patchSet.changes.filter((c: any) => typeof c === 'string' && c.trim()).map((c: string) => c.trim()).slice(0, 20)
      : [];
